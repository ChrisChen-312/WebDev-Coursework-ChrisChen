Part 1: Questions for the Client
What is your first impression of your website?
What three words would you use to describe the overall feel of the page?
Does this design match the professional, educational style you wanted for your marine science page?
How easy is the text to read on the page?
Do the images of octopuses, squids, and cuttlefish support your content well?
What is one thing you really like about the website?
What is one change you would suggest to improve the page?
Is the information organized clearly with lists and headings?
Do the colors and design feel calm and suitable for an educational site?
How well does this website meet your goal of teaching students about cephalopods?

Part 2: Client Feedback (My Client’s Responses)
My first impression is that the website looks clean, friendly, and professional.
I would describe it as clear, calm, and educational.
Yes, this feeling matches what I wanted for a marine education page.
The text is easy to read, but I think it could be a little larger.
Yes, the images are interesting and fit the topic perfectly.
I really like the organized lists and high-quality animal pictures.
I suggest making the images slightly smaller so the page is not too long.
Yes, the headings and lists make the information very easy to follow.
The light blue and soft colors feel calm and fit the ocean theme well.
This page meets my goal really well — it’s informative and visually appealing.

Part 3: What I Learned & Changes I Made
From the client’s feedback, I noticed they wanted slightly smaller images and easier-to-read text. I also saw that they enjoyed the clean, calm design.
I made these improvements:
Reduced the image width to make the page more balanced.
Increased the font size slightly for better readability.
Kept the soft ocean colors because the client liked the calm feeling.
Maintained the clear structure of headings and lists.